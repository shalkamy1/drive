...
<!-- Create Post Form -->
    ...
    <form action="{{route('category.update',$categoreis->id)}}" method="POST">
        @csrf
        <div class="form-group">
            <label for="" class="form-group my-2 text-light">Title</label>
            <input value="{{$categoreis->title}}"  class="form-control  @error('title')
is-invalid
...
<span class="text-danger"> {{$message}}</span>
        ...
        <div class="form-group">
            ...
            <input value="{{$categoreis->description}}" class="form-control @error('description')
is-invalid
